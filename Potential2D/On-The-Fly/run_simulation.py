...
BARRIER = 7 # barrier parameter for OPES  
...
min_lag,max_lag = 0.5,2 
n = 4 # how many lag times between min and max lag
...
for i in range(1,iterations):
    ...
    with open(folder+"plumed.dat","w") as file:
        ...
        # two cvs
        opes: OPES_METAD ARG=deep"""+str(i-1)+""".node-0,deep"""+str(i-1)+""".node-1 TEMP="""+str(sim_parameters["temp"])+""" PACE=500 RESTART=NO BIASFACTOR=2 CALC_WORK FILE=KERNELS BARRIER="""+str(BARRIER)+""" STATE_WFILE=RestartKernels STATE_WSTRIDE=500*10  

        # one cv
        #opes: OPES_METAD ARG=deep"""+str(i-1)+""".node-0 TEMP="""+str(sim_parameters["temp"])+""" PACE=500 BIASFACTOR=2 RESTART=NO CALC_WORK FILE=KERNELS BARRIER="""+str(BARRIER)+""" STATE_WFILE=RestartKernels STATE_WSTRIDE=500*10  
