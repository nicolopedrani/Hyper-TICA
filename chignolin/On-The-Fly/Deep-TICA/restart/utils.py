...
def plot_model_lossfunction(model,only_loss=None):

    n_eig=len(model.nn[-1].bias)

    if only_loss is not None:
        loss_train = [x.cpu() for x in model.loss_train]
        loss_valid = [x.cpu() for x in model.loss_valid]

        only_loss.plot(loss_train,'-',label='Train')
        only_loss.plot(loss_valid,'--',label='Valid')
        only_loss.set_ylabel('Loss Function')
...
def gridspec_fes(s,logweight,sim_parameters):
    ...
    fes,grid,bounds,error = compute_fes(s, weights=np.exp(logweight),
                                        ...
                                        bandwidth=sim_parameters["bandwidth"],scale_by='range', plot_max_fes=sim_parameters["plot_max_fes"])
    ...
    c.clabel()
    ax_scatter.grid()
    ax_scatter.set_xlabel(r"Deep-tica 1")
    ax_scatter.set_ylabel(r"$Deep-tica 2")
