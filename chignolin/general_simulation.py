...
STRIDE = 5000 # stride of the simulation, usually 100 which means 1/5 ps
iterations = 1 # number of iterations, I have not decided yet which criterion to stop the iterations
dt = 0.000002 # time step of simulation, in nanoseconds
Time = 5000 # in nanoseconds, time of single simulation
...
ARG = ""
count =0
#file = open('script/plumed-driver.dat', 'r').readlines() # all descriptors
file = open('script/plumed-descriptors.dat', 'r').readlines() # most important descriptors from Luigi's article
for line in file:
    if count > 7:
        if line == "\n":
            break
...
for i in range(1,iterations):
    ...
    with open(folder+"plumed.dat","w") as file:
        ...
        opes: OPES_METAD ARG=deep"""+str(i-1)+""".node-0,deep"""+str(i-1)+""".node-1 TEMP=340 PACE=500 RESTART=NO FILE=KERNELS BARRIER="""+str(BARRIER)+""" STATE_WFILE=RestartKernels STATE_WSTRIDE=500*10  
    ...
    for lag in lags:
        #random split
        # TensorDataset (x_t,x_lag,w_t,w_lag)
        dataset = create_time_lagged_dataset(X,t=t,lag_time=lag,logweights=logweight,tprime=tprime,interval=[0,n_train+n_valid])
        train_data, valid_data = random_split(dataset,[n_train,n_valid])
        train_datasets.append(train_data)
        valid_datasets.append(valid_data)
