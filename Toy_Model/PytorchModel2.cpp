...
PLUMED_REGISTER_ACTION(PytorchModel,"PYTORCH_MODEL2")
...
{ //print pytorch version
  ...
  //DEVICE: check if cuda is present
  log.printf("CUDA DEVICE COUNT: %d \n", torch::cuda::device_count());
  if (torch::cuda::is_available()) {
    device = torch::kCUDA;
    log.printf("CUDA is available! Training on GPU.\n");
  }
